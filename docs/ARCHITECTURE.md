...
| `triggers/webhook.py` | HTTP receiver: HMAC-verify `X-Hub-Signature-256`, parse push/PR → `InboundEvent`, **enqueue only** *(Alejandro, T10.1)* | `channel.base` |
| `triggers/orphan_watch.py` | on a github event (on the worker): rescan → `verify_graph_integrity` → diff orphans vs a watermark → surface each new orphan by uid, else record a silence *(Alejandro, T10.2/T10.3)* | `tools.repo_scan`, `tools.decisions` (read-only), `overlay.db`, `channel.silence` |
