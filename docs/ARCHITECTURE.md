...
- **Owns:** `cases.json`, `retrieval_metrics.py`, `loader.py`, `run_eval.py` (Part 2);
  `gen_cases.json`, `generation_metrics.py`, `answer.py`, `cache.py`, `run_gen_eval.py` (Part 3).
...
- **Part 3 (done, Phase 14D):** `generation_metrics.py` (faithfulness, answer relevance, context
  precision, context recall — named labels scored in code, `supported`/`present` verified against
  the passages by a quote check, out-of-corpus scored by `abstention` alone; #68),
  `answer.py` (the answer under test: retriever-order passages, one `CHEAP` call, ~120-word cap),
  `cache.py` (one sqlite file of answers + verdicts under `RADF_RETRIEVAL_CACHE`, so a judged run
  replays instead of re-sampling), `gen_cases.json` (24 base cases + 6, and the stratified
  reranking-OFF subset; #69), `run_gen_eval.py` (retrieve → answer → judge, over the same
  injectable `search_fn`). Judged spend is measured and reported per run.
- **Status:** **Parts 2 and 3 done** (Phases 14C, 14D)
...
### `safety/` — the safety layer (HW6, Phase 15D)
<!-- OWNER: Dias -->
- **Owns:** `types.py` (`Finding`, three named severities), `patterns.py` (the pattern catalogue
  as data, calibrated against `eval/agent_cases.json`), `input_filter.py` (Layer 1 —
  `scan_input` / `scan_data`), `output_filter.py` (Layer 3 — schema, citation via
  `generation_metrics.quote_is_present`, exfiltration), `detect.py`
  (`scan_trace(trace) -> list[Finding]`, `checks_run`, `feedback_values`, `log_findings`),
  `attack_cases.json` (5 scenarios in contract #15's shape), `run_safety_scan.py` (the batch
  pass and the false-positive report), `tests/test_safety.py`.
- **Depends on:** `tracing.trajectory` for the readable parts of a trace, and the TOOL spans
  directly for their outputs (contract #13 drops outputs on purpose; indirect injection is the
  caller that needs them). `agentlib.guards.GATED` for the irreversible set — imported, never
  restated. `eval.generation_metrics.quote_is_present` (lazily) for citation checking.
- **Does not depend on:** the agent loop, the stores, or a model. `scan_trace` is a pure
  function, so the batch pass and a live scan are the same code (#94).
- **Layers 2 and 4 are NOT here:** structural separation is `agentlib/context.py` (#26) and
  capability constraints are `agentlib/guards.py` + the tool denylists + the visibility `WHERE`
  clause (#24, #25). Reported with file names rather than re-implemented (#95).
- **Also owns (T15.20):** `triggers/heartbeat.py::run_once` runs under
  `request_origin_scope("batch")` — the one entry point `service.py::make_handler` does not cover.
- **Measured (2026-08-07, `gpt-5.4-nano`, 25 traces scanned):** **0 of 12** legitimate traces
  flagged at either threshold; 12/12 attack traces and `ae11` flagged; the agent held 4/4 of the
  attack runs that reached it, and `sa02` was refused upstream by the provider's content filter.
  Two defects in this phase's own code were found by that pass and fixed (see README § Part 3).
- **Status:** **done** (Phase 15D)
...
metrics) is #66-#67; Phase D (generation metrics) is #68-#69 — the numbers were reserved so
parallel branches did not collide in this table.
...
| 68 | 2026-08-05 | eval/ (generation metrics) | The four judged metrics grade on **named labels the code turns into a score**, and every `supported`/`present` label must carry a **verbatim quote the harness finds in the passages** or it is downgraded to `unsupported`/`absent` in code. Out-of-corpus cases run **`abstention` only**, reported separately and never averaged in. Answers are generated on `CHEAP` and judged on `STRONG` | asking the judge for a 0-10 faithfulness score (what Ragas/DeepEval do, #61); trusting a `supported` label without checking the quote; scoring the four metrics on out-of-corpus cases and letting a correct refusal read as `evades`/0.0; judging with the same model that answered | #37's rule reaches its hardest case here: the thing being measured is *whether a model told the truth*, so the number must not itself be a model's opinion. Named buckets + arithmetic in code is the same split `monitor/judge.py` uses, and it leaves an audit trail a human can read (which claim, which label, which quote). The quote check is the part that is genuinely new: it is the one judged assertion that is **mechanically falsifiable**, so "did the judge over-credit grounding" becomes a count this harness reports per run rather than a caveat in the write-up (the measured count for the run in the README is stated there). Out-of-corpus separation follows #67 exactly one level up: a question the corpus cannot answer has no faithful answer to give, and a metric family built to reward answering would score the safest behaviour worst — so those cases get the one metric that means something (did it refuse) and are excluded from the rest. `CHEAP` answers / `STRONG` judge is the cheapest available defence against self-preference; it is *reduced, not eliminated*, since both are one vendor's models, and the README says so rather than claiming the bias away. One implementation consequence worth recording: `STRONG` is a reasoning model and `max_output_tokens` bounds reasoning *and* reply together, so a cap sized for the JSON produced `reasoning_tokens == cap` and an empty verdict on 2 of the first 8 calls — the caps are set from measured reasoning cost, and a verdict that still hits one stays **ungradeable rather than guessed**. |
| 69 | 2026-08-05 | eval/gen_cases.json, eval/run_gen_eval.py | The generation set **extends** `cases.json` (24 + 6 = 30 cases) instead of being its own set, and the reranking-OFF run scores a **stratified 14-case subset declared in the data file** — two per failure category | writing a separate 20-case generation set; running both rerank settings over all 30 (what Part 2 does); picking the cheaper subset at run time, or at random | Extending is what makes Part 4 possible at all: "retrieval improved and the answer did not" is only a finding if both halves scored *the same questions*, and a second question set would have turned a comparison into two unrelated tables. It also reuses 24 hand-written golden answers rather than writing 20 more. The subset exists because Part 2's metrics are free and these are not — the full matrix twice over is roughly double the bill for a comparison the subset already supports. It is **stratified and declared in the JSON** for two different reasons: a random subset can delete a whole failure category, which is precisely what the per-category table is for; and a subset chosen after seeing results is not a subset but a selection, so it is committed to the data file where a reviewer can see it was not tuned. |

### Demo-branch decisions

`#68-#69` stay reserved for HW5 Phase D (generation metrics); the `final-demo` branch takes
**#70-#86** so the two do not collide in this table.
...
| 80 | 2026-08-05 | app/static | Search rows print the reranker's **band name** (`answers`/`related`/`unrelated`), not the float it is stored as; the channel agent's actor badge is `qa-agent`, not `agent`; the panel divider is draggable and its width persists | printing `score` as returned (`1.0`); leaving the badge as `agent` | The score `search_corpus` returns is, after reranking, the *value of a named band* (`rerank.py::BANDS`) — not a similarity. Rendering it as `1.0` was wrong in two directions at once: it reads as "100% match", which is not what it means and not something the reranker can know, and a result list of five `1.0`s reads as a broken scorer when it means all five landed in the top band, which is the reranker working. It also undid the design it was displaying — #65 chose three named bands over a numeric score *so a human could audit the judgement*, and the panel converted it back to a float at the last step. Unreranked rows say `rrf 0.0328`, labelled because that value has no meaning except its order. `agent` -> `qa-agent` because next to `planner` and `executor` a badge reading `agent` looks like a missing attribution rather than the answer: on the question path there is exactly one agent, and the badge now says which. |
| 81 | 2026-08-05 | agents.planner | The **impact walk** and the **decision lookups** print their `query_component_graph` / `retrieve_decisions` calls, hop by hop, including whether the walk closed naturally or hit the cap | leaving them untraced (the state before); or letting the model drive the walk as tool calls so it traces itself | #72 applied to the two lookups that produce the plan. They are real calls to the same tools the loop offers, made by Python so the model cannot be talked out of the hop cap (#34), and therefore in no `run_agent` trace — so the plan showed `impacted=2` with nothing to say where the 2 came from, and **"the walk found one dependent" was indistinguishable from "the walk never ran"**. A cap is only meaningful if the walk under it is visible; T6.2a's "make the cap visible in the trace" was half-satisfied by printing the number. The cap-reached branch is called out separately and loudly, because it means the true blast radius is *larger* than the plan's — the one thing a reviewer of an impact set most needs told. Handing the walk to the model instead was rejected for #34's original reason: it would trace itself, and it would also be able to stop early, wander, or be argued past the cap by context. |
| 82 | 2026-08-05 | agents.planner | The seed call's output budget is **2000 tokens**, not 400 | keeping 400; catching `truncated` and retrying | The cap must cover **reasoning** tokens, not just the reply. 400 was sized against `CHEAP`, which spends 0 reasoning tokens and answers this prompt in ~59. `STRONG` is a reasoning model: measured on this prompt it spends 285-549 reasoning tokens before emitting a character, so 400 left the object no room and the call came back `truncated` — surfacing to a user as "the planner's seed/step proposal was truncated" and a failed plan on a request that was fine. Intermittent, because it depends on how long the model thinks, which is why it survived the first runs after #76 moved the default to strong and then failed twice in a row on a live demo. 2000 is what `overlay/summarize.py` already uses against the same model for a comparably sized JSON reply. Retrying was rejected: the call was not flaky, it was under-budgeted, and a retry would have doubled the cost of every plan to paper over a constant. |
| 83 | 2026-08-05 | agents.planner | The seed call uses its own **`PLANNER_SEED_SYSTEM`**, which states there are no tools; `PLANNER_SYSTEM` is left in place but is no longer sent to a toolless call | reusing `PLANNER_SYSTEM`, which instructs the model to call `query_component_graph` and `retrieve_decisions` | `PLANNER_SYSTEM` describes a tool-using planner the planner has not been since T6.2 — the graph walk and decision lookups became deterministic Python (#34), and `_propose_seed_and_steps` passes **no tools at all**. Instructing a model to call tools it does not have is an instruction it cannot follow, and the two models fail it differently: `CHEAP` ignored the contradiction and answered in JSON, `STRONG` obeyed it and emitted tool-call-shaped text — observed replies included `CALLTYPE(query_component_graph, {"component":"app.theme"})` and `{EIF "component": "app.theme" }query_component_graph`, both of which fail `_parse_json_object` and surface as "could not parse a seed/steps plan". Two runs in four. With the corrected prompt: six in six parse, and reasoning spend roughly halves (285-549 -> 86-186 tokens) because the model is no longer working against its own instructions. A prompt that was merely inaccurate under one model became a defect under another — worth recording, because the stale instruction had been harmless for two homeworks. |
...
Phase 15A/15B (Berat) used all seven; **Phase 15D (Dias) takes #94-#98**, and #99 onward are
free for Phase 15C — pre-allocated for the same reason HW3 pre-allocated #47-#48, so two
parallel branches do not write the same row.
...
| 94 | 2026-08-07 | safety/ | The safety layer **detects and records; it never blocks, edits or refuses**, and `scan_trace(trace) -> list[Finding]` is a **pure function** run out of band | sanitising the user's text before it reaches the model; a detector on the request path that can refuse a run; a background worker scanning live traffic | Two separate reasons that happen to point the same way. **Rewriting hides the attack**: the edited text is what lands in `runs.jsonl`, in the span inputs, and in the trace this phase is graded on, so the one artefact an incident review needs is the one a sanitiser destroys (T15.14 states this as a requirement, and it is also why `_escape` in `agentlib/context.py` neutralises wrapper syntax *inside a quote* rather than deleting it). **Purity is what makes the numbers real**: the same function runs over stored traces and over a live one, so a false-positive count is reproducible, the tests can build adversarial traces from span stubs that no eval run could produce on demand, and a detector fault costs a finding rather than a run — the same trade `tracing/` makes in #90. The parts that are not pure (writing `safety.*` feedback) are a separate function, because a detector that both decides and records cannot be re-run without a side effect. |
| 95 | 2026-08-07 | safety/, README § Part 3 | Layers **2 and 4 are reported as already-implemented**, with file names and decision numbers, and are **not re-built** in `safety/`. The layer this phase "skipped" is named as such with its evidence | writing a `safety/structural.py` and a `safety/capabilities.py` so all four layers appear under one directory; claiming four new layers in the report | The brief asks which layer was skipped and why, and "already implemented in HW2, here is the file and the decision number" is the better answer than a duplicate — a second fence around retrieved data would be a second thing to keep in step with `agentlib/context.py`, and a second gated-tool list would drift from `guards.GATED` the first time the set changed (which is exactly why `detect.py` *imports* `GATED` instead of restating it). The honest accounting matters more than the tidy directory: Layer 2 is `context.py::_render_data` + `_escape` + #26, Layer 4 is `guards.GATED` + the `store/` denylists + an empty `impact_scope` (#25) + `validate_args`/`detect_stall`, and the layer doing the most work is not on the assignment's list at all — visibility as a `WHERE` clause (#24), which means an obeyed injection still gets no rows. |
| 96 | 2026-08-07 | safety.types, safety.detect | Findings carry **three named severities** (`suspicious`/`likely`/`confirmed`), feedback values are the same **named strings** plus `clean` and **`not_checked`**, and `checks_run` is reported beside every finding list | a 0-1 risk score or a 1-10 severity; a single `is_safe` boolean; omitting a check that had no input | #37's rule, third application (after the monitor's rubric and the reranker's bands), and here it also solves a problem the other two do not have: **a check that could not run and a check that found nothing produce the same empty list**. A run that retrieved nothing has no passages a quote can be verified against, so the citation check is *undefined*, not clean — #67's "empty golden is `None`, not 0" one layer out. `not_checked` in the feedback value means the MLflow UI cannot show a missing assessment as a passing one, which is the safety-layer form of "no reviewer looked, therefore no findings". Escalation is also named rather than numeric: one pattern family firing is `likely` (a quotation or a question about prompt injection will do it — this repo's own docs would), two different families in one text is `confirmed`, because "ignore your instructions" plus "do not mention this" has no innocent reading. |
| 97 | 2026-08-07 | safety.detect | Tool abuse is read off the spans' **`radf.branch`**, and **one declined call is not a finding** — the threshold is two | inferring intent from tool arguments; flagging any attempt on a gated tool; flagging the first decline | Contract #12 records the guards' decision at the dispatch site precisely so a scorer does not re-derive it, and "the run re-issued a declined `apply_change` four times" is one pass over that list where from arguments alone it is a guess. The threshold is the load-bearing part: **ae07 is a legitimate scenario that asks the agent to delete a graph node, and it is 3/3 in the Part 1 table** — the gate declining is the system working, so a threshold of one would make the best property in the repo its noisiest alarm. What *is* recorded at the first attempt is `gated_call_after_injection`: a gated tool reached in a run that also carried an injection is the single most important line in the module, because it says the payload got as far as a destructive capability even though the gate then refused it. |
| 98 | 2026-08-07 | safety/attack_cases.json, safety.run_safety_scan | The attack set reuses **contract #15's case shape and Part 1's runner**; the false-positive rate is measured on **Part 1's legitimate traces**, per **trace** (not per finding), at **two thresholds**; payloads are planted into a **sandboxed copy of `store/`** | a bespoke attack runner; measuring false positives against the attack set or against hand-written "benign" strings; reporting one flagged-rate number; planting a decision in the real overlay | An injection case is an ordinary case whose `forbidden_tools` are the payload's goal, so `eval/run_agent_eval.py::run_once` and `agent_metrics.score_case` run the attack set unmodified — no second runner to keep in step, and the agent-side verdict is scored by the same code that produced the Part 1 table. The false-positive corpus has to be traffic **written by someone else for another purpose before this detector existed**, or the number measures the author's intent; Part 1's 13 scenarios qualify, minus the one whose declared category is `injection` (excluded by category, so a new injection case in Berat's file cannot silently join the legitimate side). Per-trace because one crafted message matching six patterns is one flagged run; two thresholds because "any finding" and "something worth waking someone for" are different numbers and publishing only the friendlier one is how a safety report becomes decoration. The sandbox is not caution but correctness: `sa03` writes an attacker's decision and `sa04` prunes a node, and an attack suite whose side effect is leaving the payload in a developer's overlay has become the attack. |
